...
def get_prr_masks(secret, word, prob_f, num_bits):
  h = hmac.new(os.urandom(64), word, digestmod=hashlib.sha256)
...
class Encoder(object):
  ...
  def _internal_encode_bits(self, bits):
    ...
    uniform, f_mask = get_prr_masks(
        ...
        self.secret, int2bytes(bits), self.params.prob_f,
  ...
  def _internal_encode(self, word):
    ...
    for bit_to_set in bloom_bits:
      bloom |= (1 << bit_to_set)
    #print(bloom_bits, bin(bloom))
    #print(word, self.cohort, self.params.num_hashes, self.params.num_bloombits,bloom)
    prr, irr = self._internal_encode_bits(bloom)
    #return bloom, prr, irr
    print(dec2bin(bloom,self.params.num_bloombits), dec2bin(prr,self.params.num_bloombits), irr)
    return dec2bin(bloom,self.params.num_bloombits), dec2bin(prr,self.params.num_bloombits), irr
  ...
  def encode(self, word):
    ...
    Returns:
      An integer that is the IRR (Instantaneous Randomized Response).
    """
    bloom, prr, irr = self._internal_encode(word)
    return bloom, prr, irr
