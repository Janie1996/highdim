...
def Get_Ent(attA_prob_list):  #att information entropy
    Ent=0.0
...
def True_Dep_Graph(att_num,node_num,true_node_num,rowlist,multilist,freqrow1,freqnum1,freqrate1,freqrow2,freqnum2,freqrate2,newlist,fai_C):
    ...
    if nonBinary:
        ...
        Rel_Ent=[Get_Ent(freqrate1[i]) for i in range(att_num)]   # every att entropy

        RE_index=sorted(range(len(Rel_Ent)), key=lambda k: Rel_Ent[k])  # index of Rel_Ent (jiang xu)
        RE_index.reverse()
        ...
        for ii in range(leng_th):
            ...
            for jj in range(ii+1,leng_th):
                ...
                CorrMatrix[ii][jj]=True_Mi     # wrong??  ues the i & j
...
# #print rowlist
# #print multilist
# bit_cand_list, bit_list, bitsum_list =Get_Rappor.rappor_process(10, 2,0.1,  att_num, node_num, true_node_num, rowlist, multilist, 4)
# freqrow1, freqnum1, freqrate1, freqrow2, freqnum2, freqrate2, newlist = Get_Params.get_static_info(att_num, node_num,rowlist, multilist)
#
# TrueDepG, Truens, True_CorMat, freqrate2, freqrate1 = True_Dep_Graph(att_num, node_num, true_node_num,
#                                                                                 rowlist, multilist, freqrow1, freqnum1,
#                                                                                 freqrate1, freqrow2, freqnum2,
#                                                                                 freqrate2, newlist, 0.3)
...
def Get_Dep_Graph(att_num,node_num,origin_node_num,rowlist,multilist,num_bloom_bits, num_hash, f,bit_cand_list,bit_list,bitsum_list,fai_C):
    ...
    Rel_Ent=[Get_Ent(p_single[i])/(len(rowlist[i])-1) for i in range(att_num)]   #relative entropy
    RE_index=sorted(range(len(Rel_Ent)), key=lambda k: Rel_Ent[k])
    RE_index.reverse()   #jiang xu
